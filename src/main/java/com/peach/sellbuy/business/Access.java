...
public class Access<T> {
    ...
    public static boolean isEmpty() {
        ...
        Access<Image> imageAccess = new Access<>(Image.class);
        Access<Review> reviewAccess = new Access<>(Review.class);

        int productSize = productAccess.size();
        int userSize = userAccess.size();
        int imageSize = imageAccess.size();
        int reviewSize = reviewAccess.size();

        return productSize < 5 || userSize < 5 || imageSize < 5 || reviewSize < 5;
    }
